...
from tipsi_tools.drf.validation import validate_request
...
def test_no_error_if_no_data_is_passed_and_no_serializers_defined(req_factory: APIRequestFactory,
...
def test_raises_400_if_query_params_are_invalid(req_factory: APIRequestFactory, apiview_cls: Type[APIView]):
...
def test_raises_400_if_data_is_invalid_as_serializer(req_factory: APIRequestFactory, apiview_cls: Type[APIView]):
...
def test_has_query_params_if_passed_and_valid(req_factory: APIRequestFactory, apiview_cls: Type[APIView]):
    apiview_cls.get = validate_request(
        query_params={
            'username': serializers.CharField(allow_blank=False, required=True)
        }
...
def test_400_for_invalid_data_on_viewsets(req_factory: APIRequestFactory, viewset_cls: ViewSet):
    viewset_cls.list = validate_request(query_params={
        'username': serializers.CharField(allow_blank=False, required=True)
    })(viewset_cls.list)
...
def test_valid_data_on_viewsets(req_factory: APIRequestFactory, viewset_cls: ViewSet):
    viewset_cls.create = validate_request(data={
        'username': serializers.CharField(allow_blank=False, required=True)
    })(viewset_cls.create)
